...
# jak robimy od pewnego momentu część kodu trzeba wygasić
"""
# wygaszamy jak startujemy
# START
# czas startowy w godzinach
t_0 = 3

# Wczytanie z pliku
U = np.load("U_do_" + str(t_0) + ".npy")
u1_0 = np.load("u1_" + str(t_0) + ".npy")
u2_0 = np.load("u2_" + str(t_0) + ".npy")
u3_0 = np.load("u3_" + str(t_0) + ".npy")
u4_0 = np.load("u4_" + str(t_0) + ".npy")
...
T = 3600
h_t = 0.1
t_0 = 0                                                         # wygaszamy dla kolejnych powtórzeń
h_x = 0.1
n=int(T/h_t)
alpha = 0.025
...
############################################### tylko kiedy zaczynamy robimy tę część
# START
...
# STOP
############################# koniec części którą wykonujemy tylko za 1 razem
...
def f_grzejnik(nr_pok, sr_tem, i_x, i_y):
    if nr_pok == 1:
    # 1 pokój- 3 grzejniki
        if i_y==1 and (int(1.41/h_x)<=i_x<=int(2.51/h_x) or int(8.51/h_x)<=i_x<=int(9.61/h_x)):
            if sr_tem<=S[0]:
                #print("grzejnik działa w 1 jeden z 2")
                return v

        elif i_x == 1 and int(1.81/h_x)<=i_y<=int(2.91/h_x):
            if sr_tem <= S[0]:
    ...
    # 3 pokój
        if i_x==1 and int(1.41/h_x)<=i_y<=int(2.51/h_x):
            if sr_tem <= S[2]:
...
for k in range(n):
    ...
    for i in range(l1_x):
        for j in range(l1_y):
            if i + 1 > l1_x - 1 or j + 1 > l1_y - 1 or i - 1 < 0 or j - 1 < 0:
                ...
                v_f = f_grzejnik(1, sr_tem_w_1, i, j)
    ...
    for i2 in range(l2_x):
        for j2 in range(l2_y):
            if i2 + 1 > l2_x - 1 or j2 + 1 > l2_y - 1 or i2 - 1 < 0 or j2 - 1 < 0:
                ...
                v_f = f_grzejnik(2,sr_tem_w_2, i2, j2)
                calka_energii_cz += v_f
    ...
    for i3 in range(l3_x):
        for j3 in range(l3_y):
            if i3 + 1 > l3_x - 1 or j3 + 1 > l3_y - 1 or i3 - 1 < 0 or j3 - 1 < 0:
                ...
                v_f = f_grzejnik(3, sr_tem_w_3, i3, j3)
                calka_energii_cz += v_f
    ...
    for i4 in range(l4_x):
        for j4 in range(l4_y):
            if i4 + 1 > l4_x - 1 or j4 + 1 > l4_y - 1 or i4 - 1 < 0 or j4 - 1 < 0:
                ...
                v_f = f_grzejnik(4, sr_tem_w_4, i4, j4)
                calka_energii_cz += v_f
    ...
    calka_energii.append(calka_energii_cz*h_t*h_x*h_x)
...
# Zapis do pliku
np.save("U_do_"+ str(t_0+1)+".npy", U)
np.save("u1_"+ str(t_0+1), U_L1[:,:,-1])
np.save("u2_"+ str(t_0+1), U_L2[:,:,-1])
np.save("u3_"+ str(t_0+1), U_L3[:,:,-1])
np.save("u4_"+ str(t_0+1), U_L4[:,:,-1])

with open("data1.json", "w") as sr_tem_po_1h:
    json.dump(sr_tem_w_mieszkaniu, sr_tem_po_1h)

# Tworzenie układu podwykresów
fig, axs = plt.subplots(2, 2, figsize=(17.5, 10), sharey=True)
fig.suptitle("Mapa ciepła dla pierwszego ułożenia grzejników")
