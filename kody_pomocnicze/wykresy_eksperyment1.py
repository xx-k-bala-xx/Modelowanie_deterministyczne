...
U = np.load("U_do_" + str(3+1) + ".npy")
Ui = np.load("U_doo_" + str(3+1) + ".npy")
...
# Drugi wykres
im2 = axs[0, 1].pcolormesh(X, Y, U[:, :, 1], cmap='viridis')  # Bez globalnego vmin, vmax
...
# Tworzenie układu podwykresów
fig, axs = plt.subplots(2, 3, figsize=(17.5, 10), sharey=True)
#fig.suptitle("Mapa ciepła dla drugiego ułożenia grzejników", fontsize = 15)
fig.suptitle("")
# Tworzenie siatki współrzędnych
X, Y = np.meshgrid(xs1, y)

# Pierwszy wykres
im1 = axs[0, 0].pcolormesh(X, Y, Ui[:, :, 0], cmap='viridis')  # Bez globalnego vmin, vmax
axs[0, 0].set_xlim([min(xs1), max(xs1)])
axs[0, 0].set_ylim([min(y), max(y)])
axs[0, 0].invert_yaxis()  # Odwrócenie osi y
axs[0, 0].set_title("t=0")
...
# Drugi wykres
im2 = axs[0, 1].pcolormesh(X, Y, Ui[:, :, 1], cmap='viridis')  # Bez globalnego vmin, vmax
axs[0, 1].set_xlim([min(xs1), max(xs1)])
axs[0, 1].set_ylim([min(y), max(y)])
axs[0, 1].invert_yaxis()  # Odwrócenie osi y
axs[0, 1].set_title("t=15min")
...
#trzeci wykres
im2 = axs[0, 2].pcolormesh(X, Y, Ui[:, :, 2], cmap='viridis')  # Bez globalnego vmin, vmax
axs[0, 2].set_xlim([min(xs1), max(xs1)])
axs[0, 2].set_ylim([min(y), max(y)])
axs[0, 2].invert_yaxis()  # Odwrócenie osi y
axs[0, 2].set_title("t=30min")
...
# czwarty wykres
im3 = axs[1, 0].pcolormesh(X, Y, Ui[:, :, 8], cmap='viridis')  # Bez globalnego vmin, vmax
axs[1, 0].set_xlim([min(xs1), max(xs1)])
axs[1, 0].set_ylim([min(y), max(y)])
axs[1, 0].invert_yaxis()  # Odwrócenie osi y
axs[1, 0].set_title("t=2h ")
...
# Piąty wykres
im4 = axs[1, 1].pcolormesh(X, Y, Ui[:, :, 12], cmap='viridis')  # Bez globalnego vmin, vmax
axs[1, 1].set_xlim([min(xs1), max(xs1)])
axs[1, 1].set_ylim([min(y), max(y)])
axs[1, 1].invert_yaxis()  # Odwrócenie osi y
axs[1, 1].set_title(f"t=3h")
...
# dla 1 układu
with open("data1.json", "r") as sr_tem_po_1h:
...
# dla 2 układu
with open("data2.json", "r") as sr_tem_po_1hi:
...
plt.plot(times_short, values1, marker="o", color = "blue", label = "pokój dzienny")
plt.plot(times_short, values2, marker="o", color = "green", label = "korytarz")
plt.plot(times_short, values3, marker="o", color = "brown", label = "sypialnia")
plt.plot(times_short, values4, marker="o", color = "red", label = "łazienka")
...
plt.plot(times_short, values1i, marker="o", color = "blue", label = "pokój dzienny")
plt.plot(times_short, values2i, marker="o", color = "green", label = "korytarz")
plt.plot(times_short, values3i, marker="o", color = "brown", label = "sypialnia")
plt.plot(times_short, values4i, marker="o", color = "red", label = "łazienka")
plt.legend(title="Legenda", loc="center left", bbox_to_anchor=(1, 0.5))
plt.xlabel("Czas (HH:MM)")
plt.ylabel("Temperatura")
#plt.title("Wykres temperatury w pokojach przy drugim układzie grzejników w zależności od czasu", fontsize=20)
plt.title("")
plt.gcf().autofmt_xdate()
plt.grid()
plt.show()
...
plt.plot(times_short, values5, label="pierwszy układ", color="blue")
plt.plot(times_short, values5i, label="drugi układ", color="red")
plt.xlabel("Czas (HH:MM)")
